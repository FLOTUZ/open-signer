...
export class WebhookDispatcherService {
  ...
  static async enqueue(signatureRequest: SignatureRequest): Promise<void> {
    ...
    try {
      ...
      console.error(
        `[📬 WEBHOOK] No se pudo generar el QR para ${signatureRequest.id}:`,
        err,
      );
    }
    ...
    await prisma.auditLog
      ...
      .catch((err) =>
        console.error("Error creating webhook enqueue auditLog:", err),
      );

    console.log(
      `[📬 WEBHOOK] Job encolado para SignatureRequest ${signatureRequest.id}`,
    );
  }
  ...
  static async dispatch(job: WebhookJob): Promise<boolean> {
    ...
    console.log(
      `[📬 WEBHOOK] Intento #${attempt} para job ${job.id} → ${job.url}`,
    );
    ...
    if (success) {
      ...
      await prisma.auditLog
        ...
        .catch((err) =>
          console.error(
            "Error creating webhook dispatch success auditLog:",
            err,
          ),
        );

      console.log(
        `[✅ WEBHOOK] Job ${job.id} entregado exitosamente (HTTP ${responseCode})`,
      );
      return true;
    }
    ...
    if (hasFinallyFailed) {
      await prisma.auditLog
        ...
        .catch((err) =>
          console.error("Error creating webhook dispatch fail auditLog:", err),
        );

      console.error(
        `[❌ WEBHOOK] Job ${job.id} FALLIDO definitivamente tras ${attempt} intentos.`,
      );
    } else {
      await prisma.auditLog
        .create({
        ...
        .catch((err) =>
          console.error("Error creating webhook dispatch retry auditLog:", err),
        );

      const nextIn = nextDelayMs / 1000;
      console.warn(
        `[⚠️  WEBHOOK] Job ${job.id} falló (intento ${attempt}). Reintento en ${nextIn}s`,
      );
    }
  ...
  static async processPendingJobs(): Promise<void> {
    ...
    if (pendingJobs.length > 0) {
      console.log(
        `[📬 WEBHOOK] Procesando ${pendingJobs.length} job(s) pendientes...`,
      );
    }
