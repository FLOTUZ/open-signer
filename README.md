...
|---|---|---|---|
| `POST` | `/api/v1/signatures/request` | API Key | Crea sesión de firma (requiere indicar el `rfc` esperado), retorna `signUrl` |
...
### Validación de Identidad Estricta (RFC)

Al crear una nueva solicitud de firma (`POST /signatures/request`), es **obligatorio** proveer el campo `rfc` de la persona física o moral que debe firmar el documento. Durante el paso de completado de la firma, el servidor extrae el RFC real del certificado `.cer` utilizado por el usuario y valida que coincida exactamente con el RFC solicitado. Si los RFCs no coinciden, la firma es rechazada (HTTP 400). Esto garantiza criptográficamente que la persona que firma es exactamente la persona a la que se le solicitó.
